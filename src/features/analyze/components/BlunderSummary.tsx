...
function detectPatterns(blunders: Blunder[]): Pattern[] {
  ...
  const mostBlunderedPiece = Object.entries(pieceCount).sort(
    (a, b) => b[1] - a[1],
  )[0];
  ...
  if (worstPhase && worstPhase[1] >= 2) {
    ...
    const phaseDescriptions: Record<string, string> = {
      opening:
        "The opening is the first 10-15 moves where you develop pieces and fight for the center.",
      middlegame:
        "The middlegame is where most tactics happen. Pieces are active and kings may still be vulnerable.",
      endgame:
        "The endgame occurs when most pieces are traded. King activity and pawn structure become critical.",
    };
    const phaseAdvice: Record<string, string> = {
      opening:
        "Focus on: developing pieces quickly, controlling the center, castling early, and avoiding moving the same piece twice.",
      middlegame:
        "Before each move, check for tactics: forks, pins, skewers, and discovered attacks. Look at all your opponent's threats.",
      endgame:
        "Activate your king, push passed pawns, and calculate precisely. Every tempo matters in endgames.",
    };
    patterns.push({
      type: "phase",
      icon:
  ...
  if (missedCaptures >= 2) {
    patterns.push({
      ...
      advice:
        'Before every move, scan the board for captures. Ask: "Can I take anything? Is anything undefended?" Make this a habit.',
    });
  }
...
function generateSummary(blunders: Blunder[]): TakeawaySummary {
  ...
  if (worstPhase[1] >= 2) {
    const phaseActions: Record<string, string> = {
      opening:
        "Study opening principles: develop pieces, control center, castle early",
  ...
  if (worstPiece[1] >= 2 && piecePercent >= 30) {
    details.push(
      `Review ${pieceNames[worstPiece[0]]} tactics - ${worstPiece[1]} blunders with this piece`,
    );
  }

  if (missedCaptures >= 2) {
    details.push(
      `Practice capture awareness - you missed ${missedCaptures} free pieces`,
    );
  }
  ...
  return (
    <div className="fade-in bg-[#1e1c1a] border border-[#3d3a37] rounded-xl p-6 mt-6">
      <h3 className="m-0 mb-5 text-white text-lg font-semibold flex items-center gap-2.5">
        ...
        Pattern Analysis
        <span className="text-[0.6em] text-[#989795] font-normal ml-auto">
          Tap cards for details
        </span>
      </h3>

      {/* Pattern Cards Grid */}
      <div className="grid gap-3 grid-cols-[repeat(auto-fit,minmax(140px,1fr))] mb-5 items-start">
        {patterns.map((pattern, i) => {
          const isExpanded = expandedCard === i;
          const isHovered = hoveredCard === i;
          const outerClasses = `rounded-lg p-4 flex flex-col gap-2 cursor-pointer transition-all duration-200 ${
            isExpanded
              ? "bg-[#312e2b] border-[1px]" + pattern.color
              : isHovered
                ? "bg-[#2d2a27] border-[#4d4a47] shadow-lg translate-y-[-0.5rem]"
                : "bg-[#272522] border-[#3d3a37]"
          }`;
            ...
            >
              ...
              {isExpanded && (
                <div className="mt-2 pt-3 border-t border-[#3d3a37]">
                  ...
                  <div
                    className="rounded-md p-2 border-l-4 mt-2"
                    style={{
